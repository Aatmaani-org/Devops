...
latest_id=`aws ecr describe-images --repository-name node-js | grep -2 dev-latest | awk 'NR==2{print $1}' | tr -d '"',`
echo $latest_id
echo ---------- helm run ------------
helm upgrade --install --atomic --timeout 20s --set image.tag=$latest_id nodejss nodejs  -n qa -f values-qa.yaml
